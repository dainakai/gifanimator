...
def apply_font_scaling(root: tk.Tk, text_scale: float, min_abs_size: int = 10) -> None:
    """
    ...
    for name in font_names:
        ...
        f.configure(size=new_abs if size > 0 else -new_abs)
...
def main() -> None:
    ...
    # ここを追加：フォントだけは別途強制的に追従させる
    # 150% なら基本 ui_scale=1.5 でOK。必要なら環境変数で上書きできるようにもできます。
    apply_font_scaling(root, text_scale=ui_scale)
